...
#%%
import yaml
# with open('./Project_code/SegNext-master/config.yaml') as fh:
...
# from network.bricks import NormLayer

'''
Get Bread
'''
from network.bricks import ConvBNReLU
...
class HamBurger(nn.Module):
    def __init__(self, inChannels):
        ...
        self.lower_bread = nn.Sequential(nn.Conv2d(inChannels, C, kernel_size=1, bias=False),
                                         nn.ReLU(inplace=True)
                                        )
        self.ham = NMF2D()
        self.cheese = ConvBNReLU(C, C, 3)
        self.upper_bread = nn.Conv2d(C, inChannels, kernel_size=1, bias=False)
